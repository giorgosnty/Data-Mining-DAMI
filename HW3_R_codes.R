...
getInfoGain <- function(split_var, class_var){
  ...
  for(i in 1:nrow(attr.class.freq)){
    ...
    # *** ATTENTION !!! *** #
    entropy_split <- entropy_split + (num_obs_child.node/num_observations)*child.node.entropy
      #(child.node.freq/num_obs_child.node)*child.node.entropy
    #print(entropy_split)
  ...
  for(i in 1:number_of_columns){
    ...
    if(class(data.X[,i]) == "numeric"){
      ...
      for(j in 1:(length(x_sort_unique)-1)){
        ...
        # *** ATTENTION !!! *** #
        cutoff_point <-(data.X[j,i]+data.X[j+1,i])/2
...
my.perceptron <- function(data.X, data.Y, max.iter = 500){
  ...
  while(TRUE){
    ...
    # *** ATTENTION !!! *** #
    hypothesis <- as.matrix(X.new) %*% t(w)
...
# *** ATTENTION !!! *** #
k.fold.data.idx <- createFolds(heart.disease.db$class,num.folds)
...
for(i in 1:length(cost.parameter)){
  ...
  for(j in 1:num.folds){
    ...
    # *** ATTENTION !!! *** #
    svm.model <- svm(class~.,train.set,kernel="radial",cost=cost.parameter[i])
...
# ANSWER:The best SVM model was the result from cost parameter equals to 1.It was  0.8352874.
#(The others were 0.8050575,0.8051724,0.7642529).So we choose the highest.
